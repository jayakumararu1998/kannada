...
export default function HorizontalSmallVideoNewsWithRes({
  className,
  articleHref,
  slug,
  title = {
  ...
  return (
    <article className={cn("w-full bg-transparent", className)}>
      <NextLink
        href={href}
        className={cn(
          "flex flex-col text-inherit no-underline lg:grid lg:grid-cols-[38%_minmax(0,1fr)]",
          imageSrc.hidden && "lg:grid-cols-[minmax(0,1fr)]",
        )}
      >
        {!imageSrc.hidden && (
          <div
            className={cn(
              "order-1 relative w-full overflow-hidden bg-transparent",
        ...
        <div className="order-2 flex items-start bg-transparent pt-3 pb-2.5 [@media(min-width:480px)]:pt-[15px] [@media(min-width:480px)]:pb-3.5 lg:py-[15px] lg:pl-[25px] lg:pr-6">
          {!title.hidden && (
            <h3
              className={cn(
