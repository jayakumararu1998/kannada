...
export default function HorizontalPremiumLeftImageNews({
  className,
  articleHref,
  slug,
  ...
  return (
    <article className={cn("w-full overflow-hidden bg-transparent", className)}>
      ...
      >
        ...
        <div className="flex min-w-0 flex-col justify-start py-2 pl-[32px] pr-2">
          {!templatetype.hidden && badgeLabel && (
            <PremiumBadge
              label={badgeLabel}
              className={cn("mb-[9px] self-start", templatetype.customClass)}
            />
          )}
